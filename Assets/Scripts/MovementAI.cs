...
public class MovementAI : MonoBehaviour {
    ...
    public void UpdateMovementAI() {
        //inputs are processed right before UpdatePlayer()

        
        groundedAtStartOfFrame = controllerAI.collisions.below;//did we end last frame on the ground ?
        ...
        if (!wasPassingThroughLastFrame && controllerAI.collisions.justStartedPassingThrough) {
        ...
        controllerAI.Move(velocity * Time.deltaTime, directionalInput, playerPressedJump, isDownwardDashing, isPassingThrough);
        ...
        if (controllerAI.collisions.above || controllerAI.collisions.below) velocity.y = 0;

        //if we fell right this frame, trigger time frame during which jumping is still possible
        if (groundedAtStartOfFrame && !controllerAI.collisions.below && !playerPressedJump) timeSinceLastFall = jumpingTimeFrameAfterFalling;
    ...
    void HandlePlayerDeath() {
        if (controllerAI.collisions.isDead) {//if we've hit something lethal, trigger death
            TriggerPlayerDeath();
        }
    }

    void HandleLevelComplete() {
        if (controllerAI.collisions.hasCompletedLevel) masterManager.TriggerNextLevel();
    }

    void HandleDownwardDash() {
        if (controllerAI.collisions.downwardDashHit && isPowerDownwardDashing) {
        ...
        if (controllerAI.collisions.downwardDashHit || wallSliding) {
...
#region inputs
    ...
    public void OnJumpInputDown() {
        ...
        if (wallSliding) {
            ...
            if (wallDirX == directionalInput.x) {
                if (controllerAI.collisions.canClimb) {//if we're climbing an edge
        ...
        if (isRolling) {//if we're rolling
            if (controllerAI.collisions.below || timeSinceLastFall > 0f) {//and there's something below or we just fell
                timeSinceLastJumpInput = 0f;//if we're jumping, no need to buffer a jump
        ...
        if (((controllerAI.collisions.below && !controllerAI.collisions.crouching) || timeSinceLastFall > 0f) && !isPassingThrough) {
            timeSinceLastJumpInput = 0f;//if we're jumping, no need to buffer a jump
            velocity.y = maxJumpVelocity * currentJumpHeightMultiplier;
        }

        if (!controllerAI.collisions.below && !wallSliding && directionalInput.y == -1f && !isDownwardDashing) {//downward dash
    ...
    public void OnRollInput() {
        if (!controllerAI.collisions.below) {//trigger roll buffer only if we're in the air
            timeSinceLastRollInput = rollBufferTime;
        }

        if (controllerAI.collisions.below && !isRolling) {//start roll if we're on ground and not already rolling
            isRolling = true;
    ...
    public void OnHitInput() {
        if (controllerAI.collisions.crouching) return;//disable hitting while crouching
...
#region buffers
    void HandleRollBuffer() {
        if (controllerAI.collisions.below && !isRolling && timeSinceLastRollInput > 0f) {//roll if we're on ground, not already rolling and still in time frame
            isRolling = true;
            timeSinceStartedRolling = 0f;
            rollDirection = dirX;
            playerPressedRoll = true;
    ...
    void HandleHitWhileRollingBuffer() {
        if (timeSinceLastHitInput > 0f && !controllerAI.collisions.crouching) {
    ...
    void HandleJumpBuffer() {
        ...
        if (timeSinceLastJumpInput > 0f && !playerPressedJump && !isPassingThrough) {
            if (controllerAI.collisions.below && !controllerAI.collisions.crouching) {
                velocity.y = maxJumpVelocity * currentJumpHeightMultiplier;
            ...
            if (wallSliding) {//takes care of walljumping as well
                ...
                if (wallDirX == directionalInput.x) {
                    if (controllerAI.collisions.canClimb) {
    ...
    void CalculateVelocity() {
        ...
        if (!controllerAI.collisions.crouching) {//can't move if we're crouching
            targetVelocityX = Mathf.Abs(directionalInput.x) * dirX * currentMoveSpeed;
        }

        velocity.x = Mathf.SmoothDamp(velocity.x, targetVelocityX, ref velocityXSmoothing,
            controllerAI.collisions.below ? accelerationTimeGrounded : accelerationTimeAirborne);

        velocity.y += gravity * Time.deltaTime;
    }

    void HandleStepClimbing() {
        if (directionalInput.x == wallDirX && directionalInput.x != 0f && controllerAI.collisions.canClimb && 
    ...
    void HandleWallSliding() {
        wallDirX = controllerAI.collisions.left ? -1 : 1;
        wallSliding = false;
        if ((controllerAI.collisions.left || controllerAI.collisions.right) && !controllerAI.collisions.below && !playerPressedJump && !performedBufferedJump) {
            wallSliding = true;
    ...
    void HandleCrouching() {
        if (controllerAI.collisions.crouching) {
    ...
    void ApplyCrouchAnim(float animRatio) {//this would utlimately be replaced by an actual animation
        float scaleY = Mathf.Lerp(characterScale.y, characterScale.y * .4f, animRatio);
        transform.localScale = new Vector3(transform.localScale.x, scaleY, transform.localScale.z);
        Physics.SyncTransforms();//synchronise collider change
        controllerAI.CalculateRaySpacing();
    }
